...
export class OidcConfigurationClient extends ApiClientBase {
    ...
    constructor(baseUrl?: string, http?: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> }) {
        ...
        this.baseUrl = this.getBaseUrl("https://localhost:44330", baseUrl);
    }
...
export class InvoicesClient extends ApiClientBase {
    ...
    getAllInvoices(): Promise<InvoiceViewModel[]> {
        let url_ = this.baseUrl + "/api/Invoices";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processGetAllInvoices(_response));
        });
    }

    protected processGetAllInvoices(response: Response): Promise<InvoiceViewModel[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
